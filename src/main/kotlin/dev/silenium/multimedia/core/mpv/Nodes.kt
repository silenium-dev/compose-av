...
enum class MpvFormat(val value: Int) {
...
sealed class Node {
    abstract val format: MpvFormat

    data class Long(val value: kotlin.Long) : Node() {
        override val format: MpvFormat = MpvFormat.MPV_FORMAT_INT64
    }

    data class String(val value: kotlin.String) : Node() {
        override val format: MpvFormat = MpvFormat.MPV_FORMAT_STRING
    }

    data class Flag(val value: Boolean) : Node() {
        override val format: MpvFormat = MpvFormat.MPV_FORMAT_FLAG
    }

    data class Double(val value: kotlin.Double) : Node() {
        override val format: MpvFormat = MpvFormat.MPV_FORMAT_DOUBLE
    }

    data class List(val nodes: kotlin.collections.List<Node>) : Node() {
        override val format: MpvFormat = MpvFormat.MPV_FORMAT_NODE_ARRAY

        constructor(elements: Array<Node>) : this(elements.toList())

        val asArray: Array<Node>
            get() = nodes.toTypedArray()
    }

    data class Map(val entries: kotlin.collections.Map<kotlin.String, Node>) : Node() {
        override val format: MpvFormat = MpvFormat.MPV_FORMAT_NODE_MAP

        data class Entry(val key: kotlin.String, val value: Node) {
            constructor(entry: kotlin.collections.Map.Entry<kotlin.String, Node>) : this(entry.key, entry.value)
        }

        @Suppress("unused")
        constructor(entries: Array<Entry>) : this(entries.associate { it.key to it.value })
        constructor(vararg entries: Pair<kotlin.String, Node>) : this(entries.toMap())

        val asArray: Array<Entry>
            get() = entries.map(::Entry).toTypedArray()
    }

    data class ByteArray(val value: kotlin.ByteArray) : Node() {
        override val format: MpvFormat = MpvFormat.MPV_FORMAT_BYTE_ARRAY
    ...
    data object None : Node() {
        override val format: MpvFormat = MpvFormat.MPV_FORMAT_NONE
    }
}

fun nodeMapOf(vararg entries: Pair<String, Node>): Node.Map = Node.Map(entries = entries)
fun nodeListOf(vararg elements: Node) = Node.List(elements = elements.toList().toTypedArray())
fun nodeByteArrayOf(vararg byte: Byte) = Node.ByteArray(byteArrayOf(*byte))
